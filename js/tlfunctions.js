var tlFunc = {
  initial: function (id, name, print) {
    document.querySelector("nav").innerHTML += `<a href='#${id}'> <svg><use xlink:href='#angle-right'></svg>${name.split(" ")[0]}</a>`;
    document.querySelector("body").innerHTML +=
      `<section class='timeline ${print}' id='${id}'>
  ...
  education: function (key, { prettyName, logo, noPrint, entries }) {
    ...
    for (const { degree, date, completed, institution, subinstitution } of Object.values(entries)) {
      document.querySelector(`#${key}`).innerHTML +=
        `<div>${this.tlImage(logo)}<div>
            <h4>${degree} <span> ${date} ${
  ...
  awards: function (key, { prettyName, logo, noPrint, entries }) {
    this.initial(key, prettyName, noPrint);
    for (const { award, date, institution, subinstitution } of Object.values(entries)) {
      document.querySelector(`#${key}`).innerHTML +=
        `<div>${this.tlImage(logo)}<div>
  ...
  skills: function (key, { prettyName, logo, noPrint, entries }) {
    ...
    for (const { prettyName, list } of Object.values(entries)) {
      ...
      for (const { icon, text } of Object.values(list))
        htmltext += `<span>${icon != "" ? `<svg><use xlink:href='/img/skills.svg#${icon}'/></svg>` : ''} ${text}</span>`;
      document.querySelector(`#${key}`).innerHTML +=
        `<div>${this.tlImage(logo)}<div>
            <h4>${prettyName}</h4>
  ...
  research: function (key, { prettyName, logo, noPrint, entries }) {
    this.initial(key, prettyName, noPrint);
    for (const { position, start, end, institution, subinstitution, projects, } of Object.values(entries)) {
      var htmltext = "";
  ...
  teaching: function (key, { prettyName, logo, noPrint, entries }) {
    ...
    for (const { list, organization } of Object.values(entries)) {
      var htmltext = "";
      for (const { position, title, course, date } of Object.values(list)) htmltext += `<h5>${position} ${title != "" ? `(${title}) in` : "for"} ${course}<span>${date}</span></h5>`;
      document.querySelector(`#${key}`).innerHTML += `<div>${this.tlImage(logo)}<div><h4>${organization}</h4>${htmltext}</div></div>`;
    }
  },
  ...
  projects: function (key, { prettyName, logo, noPrint, entries }) {
    ...
    for (const { title, image, text } of Object.values(entries)) {
      document.querySelector(`#${key}`).innerHTML += `<div>${this.tlImage(logo)}<div><h4>${title}</h4><div><img src='${image}' alt='' onerror='this.style.display=none'><p>${text}</p></div></div></div>`;
    }
  },
  ...
  publications: function (key, { prettyName, logo, noPrint, entries }) {
    ...
    for (const { prettyName, list } of Object.values(entries)) {
      ...
      for (const { prettyName, items } of Object.values(list)) {
        var htmltext1 = "";
        for (const { url, citationhtml } of Object.values(items)) htmltext1 += `<p class='bull-ind'><a href='${url}'>${citationhtml}</a></p>`;
        htmltext += `<h5 class='noprint'>${prettyName}</h5>${htmltext1}`;
      }
      document.querySelector(`#${key}`).innerHTML += `<div>${this.tlImage(logo)}<div><h4>${prettyName}</h4>${htmltext}</div></div>`;
    }
  },

  presentation: function (key, { prettyName, logo, noPrint, entries }) {
    this.initial(key, prettyName, noPrint);
    for (const { conference, date, titlehtml } of Object.values(entries)) document.querySelector(`#${key}`).innerHTML += `<div>${this.tlImage(logo)}<div><h4>${conference}<span>${date}</span></h4><h5 class='entry-description'>${titlehtml}</h5></div></div>`;
  },
  ...
  references: function (key, { prettyName, logo, noPrint, entries }) {
    this.initial(key, prettyName, noPrint);
    for (const { name, email, position, institution } of Object.values(entries)) {
      document.querySelector(`#${key}`).innerHTML +=
        `<div>${this.tlImage(logo)}<div>
      ...
      for (const { prettyName, content, icon, link } of Object.values(list)) {
        htmltext += `
          <h5 class='${ind}'> ${prettyName != "" ? prettyName + ":" : ""}
            ...
            <a href='${link}'>
              ${content != "" ? content + "&nbsp;" : ""}
              ${icon != "" ? `<svg><use xlink:href='/img/icons.svg#${icon}'/></svg>` : ""}
            </a>
          </h5>`;
      }
      document.querySelector(`#${key}`).innerHTML += `<div>${this.tlImage(logo)}<div><h4>${prettyName}</h4>${htmltext}</div></div></div>`;
    }
  },
