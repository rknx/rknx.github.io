var tlFunc = {
    ...
    for (const {degree,date,completed,institution,subinstitution} of Object.values(entries)){
      $( `#${key}`).append(
        `<div>
          ...
          <div>
            <h4>${degree} <span> ${date} ${((completed == false) ?  "(expected)":"")}</span></h4>
  ...
  research: function(key, {prettyName,logo,noPrint,entries}) {
    ...
    for (const {position,start,end,institution,subinstitution,projects} of Object.values(entries)){
      var htmltext="";
      for (const list of Object.values(projects)) htmltext+=`<p class='bull-ind'>${list}</p>`;          
      $(`#${key}`).append(`
        <div>
          <div><div><svg><use xlink:href='/img/icons.svg#${logo}'/></svg></div></div>
          <div>
  ...
  affiliations: function(key, {prettyName,logo,noPrint,entries}) {
    ...
    for (const {organization,position} of Object.values(entries)){
      var htmltext="";
      for (const {role,start,end} of Object.values(position)) htmltext+=`<h5>${role}<span>${start} – ${end}</span></h5>`;          
      $(`#${key}`).append(`
        <div>
          <div><div><svg><use xlink:href='/img/icons.svg#${logo}'/></svg></div></div>
          <div><h4>${organization}</h4>${htmltext}</div>
